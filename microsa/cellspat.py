...
def cell_cell_type_neigbor(cells_neigh_index, cell_type_list, cell_type):
    ...
    cell_cell_type = [[i for i in cells_neigh_index[j] if cell_type_list[i] == cell_type] for j in
                      np.arange(len(cells_neigh_index))]
...
def cell_fibs_neigh_length(executed_fibs, cell_fibs_neigh_index):
    ...
    cell_fibs_neigh_length = []

    for j in np.arange(len(cell_fibs_neigh_index)):

        cell_fibs_length = []

        cell_fibs_mean = np.mean(cell_fibs_length)

        cell_fibs_neigh_length.append(cell_fibs_mean)

        for i in np.arange(len(cell_fibs_neigh_index[j])):
            cell_fibs = executed_fibs['props_pruned'][i].area

            cell_fibs_neigh_length.append(cell_fibs_length)
...
def cell_fibs_neigh_strightness(executed_fibs, cell_fibs_neigh_index):
    ...
    for j in np.arange(len(cell_fibs_neigh_index)):
        ...
        for i in cell_fibs_neigh_index[j]:
            fibs_strightness = dist_cheb.chebyshev(executed_fibs['props_pruned'][i].coords[np.lexsort(
